...
public class RotationalPower implements IFormula {
    ...
    public double compute(String variable, String[] values) {
        ...
        } else if (variable.equalsIgnoreCase("Torque")) {
            speed = Double.parseDouble(values[0]);
            efficiency = Double.parseDouble(values[1]);
            power = Double.parseDouble(values[2]);
            computeTorque();
            return torque;
        ...
        } else if (variable.equalsIgnoreCase("Efficiency")) {
            power = Double.parseDouble(values[0]);
            torque = Double.parseDouble(values[1]);
